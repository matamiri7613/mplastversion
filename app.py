...
def dashboard_summary():
    """Return role-specific summary data for the dashboard"""
    if 'username' not in session:
        return jsonify({'status': 'error', 'message': 'Not logged in'}), 401
    ...
    try:
        ...
        # Common counters used across roles
        total_items = len(existing_data)
        active_users = len(set([entry.get('submitter') for entry in existing_data if entry.get('submitter')]))
        
        summary_data = {}
        
        # Primary reviewer (user1) dashboard summary
        if user_role == 'primary_reviewer':
            # All entries assigned to this reviewer
            assigned_entries = [entry for entry in existing_data if entry.get('primary_reviewer') == username]
            
            # Entries needing initial review
            initial_reviews = len([entry for entry in assigned_entries 
                                 if entry.get('review_stage') == 'initial'])
            
            # Entries needing final decision (back from secondary reviewer)
            pending_final = len([entry for entry in assigned_entries 
                               if entry.get('review_stage') == 'pending_final'])
            
            # Combined pending reviews (initial + returned from secondary reviewer)
            pending_reviews = initial_reviews + pending_final
            
            # Entries approved by this reviewer
            approved_items = len([entry for entry in assigned_entries 
                                if entry.get('final_decision') is True])
            
            # Entries rejected by this reviewer
            rejected_items = len([entry for entry in assigned_entries 
                                if entry.get('final_decision') is False])
            
            # Entries in secondary review stage
            in_secondary_review = len([entry for entry in assigned_entries 
                                     if entry.get('review_stage') == 'with_secondary'])
            
            # Current week submissions
            current_time = int(time.time())
            one_week_ago = current_time - (7 * 24 * 60 * 60)
            recent_submissions = len([entry for entry in assigned_entries 
                                    if entry.get('submission_timestamp', 0) > one_week_ago])
            
            summary_data = {
                'pending_reviews': pending_reviews,  # Combined initial + pending_final
                'initial_reviews': initial_reviews,  # Initial reviews only (for reference)
                'pending_final_decisions': pending_final,  # Items back from secondary reviewer
                'approved_items': approved_items,
                'rejected_items': rejected_items,
                'in_secondary_review': in_secondary_review,
                'total_assigned': len(assigned_entries),
                'recent_submissions': recent_submissions
            }
        
        # Secondary reviewer dashboard summary
        elif user_role == 'secondary_reviewer':
            # All entries assigned to this secondary reviewer
            assigned_entries = [entry for entry in existing_data 
                              if entry.get('secondary_reviewer') == username]
            
            # Entries needing acknowledgment
            pending_secondary_reviews = len([entry for entry in assigned_entries 
                                          if entry.get('review_stage') == 'with_secondary' and not entry.get('acknowledged')])
            
            # Entries acknowledged by this reviewer
            completed_reviews = len([entry for entry in assigned_entries 
                                   if entry.get('acknowledged') is True])
            
            # Calculation for weekly completion rate (if any)
            current_time = int(time.time())
            one_week_ago = current_time - (7 * 24 * 60 * 60)
            
            # Entries assigned in the last week
            recent_assignments = [entry for entry in assigned_entries 
                                if entry.get('sent_to_secondary_timestamp', 0) > one_week_ago]
            
            # Completed reviews in the last week
            recent_completions = [entry for entry in recent_assignments 
                                if entry.get('acknowledged') is True]
            
            # Calculate completion rate
            weekly_completion_rate = 0
            if recent_assignments:
                weekly_completion_rate = round((len(recent_completions) / len(recent_assignments)) * 100)
            
            summary_data = {
                'pending_secondary_reviews': pending_secondary_reviews,
                'completed_reviews': completed_reviews,
                'total_assigned': len(assigned_entries),
                'weekly_completion_rate': weekly_completion_rate
            }
        
        # Submitter dashboard summary  
        elif user_role == 'submitter':
            # All entries submitted by this user
            submitted_entries = [entry for entry in existing_data 
                               if entry.get('submitter') == username]
            
            # Approved submissions
            approved_items = len([entry for entry in submitted_entries 
                                if entry.get('final_decision') is True])
            
            # Rejected submissions
            rejected_items = len([entry for entry in submitted_entries 
                                if entry.get('final_decision') is False])
            
            # Pending submissions (not yet complete)
            pending_items = len([entry for entry in submitted_entries 
                               if entry.get('review_stage') != 'completed'])
            
            # In progress - broken down by stage
            initial_review = len([entry for entry in submitted_entries 
                                if entry.get('review_stage') == 'initial'])
            
            secondary_review = len([entry for entry in submitted_entries 
                                  if entry.get('review_stage') == 'with_secondary'])
            
            pending_final = len([entry for entry in submitted_entries 
                               if entry.get('review_stage') == 'pending_final'])
            
            # Calculate approval rate
            approval_rate = 0
            completed_items = approved_items + rejected_items
            if completed_items > 0:
                approval_rate = round((approved_items / completed_items) * 100)
            
            summary_data = {
                'submitted_items': len(submitted_entries),
                'approved_items': approved_items,
                'rejected_items': rejected_items,
        ...
        # Return the role-specific summary data
...
@app.route('/reviewer/print_view/<submission_id>')
def print_view(submission_id):
    """Generate a printable view for a submission."""
    if 'username' not in session or session['role'] not in ['primary_reviewer', 'secondary_reviewer']:
        flash('شما دسترسی لازم را ندارید', 'error')
        return redirect(url_for('index'))
    ...
    try:
        ...
        if not submission:
            flash('درخواست مورد نظر یافت نشد', 'error')
            return redirect(url_for('reviewer_dashboard'))
        
        # Check if the user has access to this submission
        current_user = session['username']
        if submission.get('primary_reviewer') != current_user and submission.get('secondary_reviewer') != current_user:
            flash('شما دسترسی به این درخواست را ندارید', 'error')
            return redirect(url_for('reviewer_dashboard'))
            
        # Render the printable template
        return render_template('print_view.html', submission=submission)
        
    except Exception as e:
        flash(f'خطا در بارگیری اطلاعات: {str(e)}', 'error')
        return redirect(url_for('reviewer_dashboard'))
